...
def analysis_ChineseMedicine(upl_tongue_path, user_id):
    ...
    """
    tongue_features, scale_tongue, tongue_features_all = tongueHist.getVec(
        upl_tongue_path)  # 调用定义的getVec()方法，获取舌体的特征值、各区域所占比值、各区域的特征值
    ...
    health_value = Main(features_all, scale_tongue,
                        user_history_healthy=user_history_healthy)  # 调用定义的Main函数，计算各区域以及整体的健康值
