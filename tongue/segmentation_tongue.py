...
def seg_tongue(img_input, hist = False):
    ...
    # 获取最大连通域
    contours, hierarchy = cv2.findContours(img, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE) # 使用使用cv2.findContours()函数来查找舌体的轮廓，并返回轮廓一系列的坐标值
    maxSize = 0 # 初始化最大轮廓面积的编号，即最大连通区域的编号
    for i in range(len(contours)):
        if cv2.contourArea(contours[maxSize]) < cv2.contourArea(contours[i]): # 使用cv2.contourArea()函数计算轮廓的面积，如果轮廓maxSize的面积小于轮廓i的面积
    ...
    # 获取mask
    if hist:
        kidney_mask, lung_mask, spleen_mask, liver_left_mask, liver_right_mask = segmentation.viscera_split(mask) #调用定义的viscera_split()函数，将舌体划分为各脏器对应的区域
